...
# 使用例
one_hand_df = process_csv("10.csv")
...
# Tensor Boardコールバック
tb_cb = keras.callbacks.TensorBoard(log_dir='model_log/relu_9000a3', histogram_freq=1, write_graph=True)
model.fit(x_train, y_train_reshape, epochs=30, batch_size=32, validation_split=0.2,callbacks=[tb_cb])
# 計測終了時間
end_time = time.time()

# 経過時間を表示
elapsed_time = end_time - start_time
print(f"Training completed in {elapsed_time} 秒.")
model.save('saved_model_reversi/my_model9000a3')
print('Output saved')

# テストデータの処理関数
def process_test_csv(file_path):
    csv_data = pd.read_csv(file_path)
    move_sequences = csv_data.iloc[:, -1]  # 最終列から手のデータを抽出
    extract_one_hand = move_sequences.str.extractall(r'(..)')
    ...
    # 手の座標に変換する関数
    def convert_move(v):
        left_conv_table = {chr(i): i - 96 for i in range(97, 105)}  # a:1, b:2, ..., h:8
        l = left_conv_table[v[:1]]
        r = int(v[1:])
        return np.array([l - 1, r - 1], dtype='int8')
...
# 使用例
test_df = process_test_csv("2021W.csv")

# 入力データ x_test の作成
test_boards_black = []
test_boards_white = []
for index, row in test_df.iterrows():
    process_tournament(row)
    test_boards_black.extend(boards_black)
    test_boards_white.extend(boards_white)

x_test = np.concatenate([test_boards_black, test_boards_white])
...
for index, row in test_df.iterrows():
    process_tournament(row)
    test_moves_black.extend(moves_black)
    test_moves_white.extend(moves_white)

y_test = np.concatenate([test_moves_black, test_moves_white])
y_test_reshape = y_test.reshape(-1, 64)  # 各手を1次元64要素に変換

# モデルの読み込み
model = keras.models.load_model('saved_model_reversi/my_model9000a3')

# 評価
test_loss, test_accuracy = model.evaluate(x_test, y_test_reshape)
