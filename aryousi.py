...
# Tensor Boardコールバック
tb_cb = keras.callbacks.TensorBoard(log_dir='saved_model_reversi/my_modelryoua3', histogram_freq=1, write_graph=True)
model.fit(x_train, y_train_reshape, epochs=10, batch_size=32, validation_split=0.2,callbacks=[tb_cb])
model.save('saved_model_reversi/my_modelryoua3')
print('Output saved')


def representative_data_gen():
    for input_value in x_train[:100]:  # x_trainの一部を使用（大きすぎるとメモリ不足になる）
        input_value = input_value.astype(np.float32)
        input_value = np.expand_dims(input_value, axis=0)  # バッチ次元を追加
        yield [input_value]

# モデルの保存先を指定
saved_model_dir = 'saved_model_reversi/my_modelryoua3'
...
converter.inference_input_type = tf.int8  # 入力を int8 に
converter.inference_output_type = tf.int8  # 出力を int8 に
...
print(f"量子化済みモデルを保存しました: {quantized_model_path}")
# x_trainの形状を確認
...
# 代表データセットの形状を確認
for input_value in representative_data_gen():
    print("Representative data shape:", np.array(input_value).shape)
    break
