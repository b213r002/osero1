...
def debug_print_board(board_black, board_white):
    print("Current Board:")
    for y in range(8):
        row = []
        for x in range(8):
            if board_black[y][x] == 1:
                row.append("●")  # 黒石
            elif board_white[y][x] == 1:
                row.append("○")  # 白石
            else:
                row.append("・")  # 空白
        print(' '.join(row))
...
def process_tournament(df):
    ...
    if df["tournamentId"] != b_tournamentId:
        ...
        is_black_turn = True
        print("\n初期状態を確認\n")
        debug_print_board(board_black, board_white)  # 初期状態を確認
    else:
        # ターンを切り替える
        is_black_turn = switch_turn(is_black_turn)
        if is_pass(is_black_turn, board_black, board_white): # パスすべき状態か確認する
            print("\nPass detected.\n")
            debug_print_board(board_black, board_white)  # パス時の状態を確認
            print("\n")
    ...
    if is_black_turn:
        ...
        debug_print_board(board_black, board_white)  # 黒番の状態を確認
    else:
...
model.fit(x_train, y_train_reshape, epochs=5, batch_size=32, validation_split=0.2,callbacks=[tb_cb])
